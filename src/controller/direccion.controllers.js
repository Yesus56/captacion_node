...
function getDireccion(req, res) {
  ...
  Direccion.findOne({ where: { id_persona: persona } })
    .then(result => {
      if (_.isEmpty(result)) {
        return res
          .status(403)
          .json(Res(false, "su data no fue guardada", token));
      }
...
function updateDireccion(req, res) {
  ...
  Direccion.update(datos, { where: { id_persona: persona } })
    .then(result => {
      if (_.isEmpty(result)) {
        return res
          .status(403)
...
export { getDireccion, insertDireccion, updateDireccion };
